...
假设类型参数的下限为$L_1 , \ldots , L_n$ ，上限为$U_1,\ldots, U_n$。如果每个实际类型参数，参数化类型都是 *符合其边* 的,即$\sigma L_i <: T_i <: \sigma U_i$ 其中 $\sigma$ 是替换$[ a_1 := T_1 , \ldots , a_n := T_n ]$.   
...
*带注释的类型*$T$ $a_1, \ldots, a_n$将[注释](11-annotations.html#user-defined-annotations)$a_1 , \ldots , a_n$附加道$T$类型。
...
1.  $T$ 类型的*成员绑定*是
   ...
   类型投影`S#T`的定义是`S`中的类型`T`的成员绑定$d_T$。在这种情况下，我们还说`S#T`是由$d_T$定义的.
