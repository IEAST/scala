...
类成员的类型取决于成员的引用方式。主要有三个概念，即：
1. $T$类型的基本类型集的概念。
1. 从前缀类型$S$可见的类$C$中的类型$T$的概念。  
1. 类型$T$的成员绑定集的概念.

这些概念的详细定义如下：
1. 类型$T$的基类型集定义如下:
  - $C$是类型$C$以及其父类$T_1 , \ldots , T_n$的基本类型，同时也是复合类型`$T_1$ with … with $T_n$ { $R$ }`的基本类型。
  - 混别类型的基类型是其别名的类型的基类型。
  - 抽象类型的基类型是其上限的基类型。
  - 参数化类型`$C$[$T_1 , \ldots , T_n$]`的基类型是$C$类型的基类型，其中$C$的每一个类型参数 $a_i$ 都被相应的参数类型 $T_i$ 替换。
  - 单例类型`$p$.type`的基本类型是$p$类型的基本类型。
  - 复合类型`$T_1$ with $\ldots$ with $T_n$ { $R$ }`的基类型是所有$T_i$的基类的 *缩减合并*。这意味着：让设合集$\mathscr{S}$ 成为所有$T_i$的基类型的集合。如果$\mathscr{S}$包含同一类的多个类型实例，比如说`$S^i$#$C$[$T^i_1 , \ldots , T^i_n$]`$(i \in I)$，则所有这些实例都被一个与其他一致的实例替换。如果不既存这样的实例，则会出错。由此可见，缩减合并（如果存在）会产生类类型的集合，其中不同类型是不同类的实例。
  - 类型选择`$S$#$T$`的基类型确定如下：如果$T$是别名或抽象类型，则前面的子句就会被应用。否则$T$必须是(可能还是参数化)类类型，它在某个类$B$中定义。然后`$S$#$T$`的基类型是从前缀类型$S$看到的$B$中的$T$的基本类型。
  - 既存类型`$T$ forSome { $Q$ }`的基类型都是`$S$ forSome { $Q$ }`类型，其中$S$是$T$的基本类型。

1. *只有前缀类型$S$具有类$C$的类型实例作为基类型* ,比如`$S'$#$C$[$T_1 , \ldots , T_n$]`，从某些前缀类型$S$看到的类$C$中的$T$概念才有意义。然后我们定义如下:
    - 如果 `$S$ = $\epsilon$.type`, 则从 $T$看到的$C$中的$T$是$T$本身。
    - 或者，如果$S$是既存类型`$S'$ forSome { $Q$ }`，并且从 $S'$ 看到的$C$中的$T$是$T'$。那么从$S$中看到的$T$中的$C$是`$T'$ forSome {$\,Q\,$}`.
    - 或者, 如果 $T$ 是某个类$D$的第$i$个类型参数, 那么
        - 如果 $S$ 有一个基本类型 `$D$[$U_1 , \ldots , U_n$]`,`[$U_1 , \ldots , U_n$]`是类型参数 , 那么$S$中看到的$C$中的$T$是$U_i$
        - 或者，如果$C$定义在$C'$类中，那么$S$中看到的$C$的$T$与$S'$中看到的$C'$的$T$相同。
        - 或者，如果$C$没有在其他类中定义，那么$S$中看到的$C$中的$T$就是$T$本身。
    - 或者，如果$T$是某个类$D$的单例类型` $D$.this.type`，那么
        - 如果$D$是$C$的子类，$S$的基类型中有$D$的类型实例，那么$S$中看到的$C$中的$T$就是$S$。
        - 或者，如果$C$定义在$C'$类中，那么$S$中看到的$C$中的$T$与$S'$中看到的$C'$中的$T$相同。
        - 或者,如果$C$没有在其他类中定义，那么$S$中看到的$C$中的$T$就是$T$本身。
    - 如果$T$是其他类型，那么将在所有其类型组件执行以上所描述的映射。

  如果$T$是可能参数化的类类型，其中$T$的类定义在某些其他类$D$中，而$S$是某种前缀类型，那么我们"从$S$中看到的$T$"就是"从$S$中看到$D$中的$T$"的简写。

1.  $T$ 类型的*成员绑定*集合是
   1.所有绑定$d$使得在$T$的基类型中既存某个类$C$的类型实例，并且在$C$中既存定义或声明$d'$,$d$是将$d'$中的类型$T'$替换为从$T$中看到的$C$中的$T'$得到的结果。
   2. 类型[修饰](#复合类型)的所有绑定，如果有的话。
...
| 名称         | 象征           | 解释                            |
|-------------|----------------|--------------------------------|
| 恒等         | $T \equiv U$   | $T$和$U$在所有情况下都可以互换。   |
| 一致性       | $T <: U$       | 类型$T$符合(是的子类型)类型$U$     |
| 弱一致性      | $T <:_w U$    | 增加原始数字类型的一致性。         |
| 兼容性       |                | 类型$T$在转换后符合类型$U$。      |
...
类型之间的恒等$（\ equiv）$关系是最小的一致性[^一致性]，具有以下特点：
- 如果 $t$ 由类型别名 `type $t$ = $T$`定义, 那么 $t$ 等价于$T$.
- 如果路径 $p$ 有一个单例类型 `$q$.type`, 那么 `$p$.type $\equiv q$.type`.
- 如果 $O$ 是由对象定义的,而且 $p$ 是一个只包含包或对象的选择器并以$O$结尾的路径, 那么 `$O$.this.type $\equiv p$.type`.
- 两个[复合类型](#复合类型)是相等的条件是，如果它们的组件的序列的元素相等，以相同的顺序出现，并且它们的修饰是相等。如果两个修饰绑定相同的名称，并且每个声明的实体的修饰符，类型和边界在也是相等的，则两个修饰是相等的。
- 两个[方法类型](#方法类型)是相等的条件是:
    - 都是隐式的,或者两者都不是隐式[^隐式]的;
    - 它们具有相同的结果类型;
    - 它们有相同数量的参数;
    - 相应的参数具有等价的类型。请注意，参数的名称对于方法类型的相等无关紧要。
- 如果两个[多态方法类型](#多态方法类型)具有相同数量的类型参数，并且在将一组类型参数重命名为另一组之后，结果类型以及相应的下限和上限是相等的的 多态方法类型也是相等的。
- 如果两个[既存类型](#既存类型)具有相同数量的量词，并且在将一个类型量词列表替换为另一个之后，量化类型以及相应量词的下限和上限是相等的。则它们是相等的。
- 两个[类型构造函数](#类型构造函数)是相同的条件是，如果它们具有相同数量的类型参数，并且在将一个类型参数列表替换为另一个之后，结果类型以及方差，相应类型的下限和上限也是相等的。

[^一致性]：一致性是一种上下文构成的等价关系。
[^隐式]：如果定义它的参数部分以`implicit`关键字开头，则方法类型是隐式的。

### 一致性

一致性关系$(<:)$是满足以下条件的最小传递关系。

- 一致性包括相等。如果$T = U$，那么$T <: U$。
- 对于任意值类型$T$，有`scala.Nothing <: $T$ <: scala.Any`.
- 对于任意类型构造函数$T$(具有任意数量的类型参数),有`scala.Nothing <: $T$ <: scala.Any`.
- 对于任意类型$T$ ,如果 `$T$ <: scala.AnyRef` ，有 `scala.Null <: $T$`.
- 类型变量或抽象类型 $t$ 符合其上限，其下限符合 $t$.
- 类类型或参数化类型与其任何基类型一致。
- 单例类型 `$p$.type` 与$p$路径的类型一致。
- 单例类型 `$p$.type` 与`scala.Singleton`类型一致.
- 如果$T$和$U$一致，则类型映射`$T$ T$`和`$U$ T$`一致。
- 参数化类型`$T$[$T_1$，…，$T_n$]`与`$T$[$U_1$，…，$U_n$]`一致的条件是$i \in { 1 , \ldots , n }$:
 1. 如果$T$的第$i$个类型参数声明为协变，则 $T_i <: U_i$.
 1. 如果$T$的第$i$个类型参数声明为逆变的，那么  $U_i <: T_i$.
 1. 如果$T$的第$i$个类型参数既不是协变的，也不是逆变的，那么 $U_i \equiv T_i$.
- 复合类型 `$T_1$ with $\ldots$ with $T_n$ {$R\,$}` 的每个组件和类型 $T_i$一致.
- 如果$i \in { 1 , \ldots , n }$$T <: U_i$ 为$R$ 中的任意类型或值$x$的绑定$d$，$T$中存在一个包含$d$的$x$的成员绑定，则$T$和复合类型 `$U_1$ with $\ldots$ with $U_n$ {$R\,$}`一致.
- 如果既存类型`$T$ forSome {$\,Q\,$}`的[斯柯伦化](#既存类型)和$U$一致，既存类型`$T$ forSome {$\,Q\,$}`和$U$一致.
- 如果$T$和`$U$ forSome {$\,Q\,$}`的某一个[类型实例](既存类型)一致，则$T$类型和既存类型`$U$ forSome {$\,Q\,$}`一致。
- 如果$T_i \equiv T_i'$$i \in { 1 , \ldots , n}$，$U$和$U'$一致，那么方法类型$(p_1:T_1 , \ldots , p_n:T_n) U$和$(p_1':T_1' , \ldots , p_n':T_n') U'$一致。

- 多态类型$[a_1 >: L_1 <: U_1 , \ldots , a_n >: L_n <: U_n] T$与复合多态类型$[a_1 >: L_1' <: U_1' , \ldots , a_n >: L_n' <: U_n'] T'$一致的前提是：假设$L_1' <: a_1 <: U_1' , \ldots , L_n' <: a_n <: U_n'$，$T <: T'$，$L_i <: L_i'$,$U_i' <: U_i$，$i \in \{ 1 , \ldots , n \}$.

- 类型构造函数$T$ 和 $T'$ 遵循类似的规律。我们通过类型参数子句$[a_1 , \ldots , a_n]$和  $[a_1' , \ldots , a_n']$来区分$T$和$T'$，其中 a_i$或$a_i'$可以包含差异标注，高阶类型参数子句和边界。那么$T$和$T'$一致的条件是，任意列表$[t_1 , \ldots , t_n]$ --具有声明的差异，边界和高阶类型参数子句-- 是$T'$的有效类型参数，同时也是$T$的有效参数类型，并且有$T[t_1 , \ldots , t_n] <: T'[t_1 , \ldots , t_n]$。这将导致：
    - $a_i$的边界必须弱于$a'_i$声明的相应边界。
    - $a_i$的方差必须与$a'_i$的差异匹配，其中协变匹配协变，逆变匹配逆变，任何差异要和无差异一致。
    - 同样地，这些限制适用于相应的高阶类型参数子句$a_i$和$a'_i$。_
在以下的某些条件下，类类型$C$的复合类型的声明或定义将包含另一个类类型$C'$的符合类型的同名声明：
- 如果$T <: T'$.一个值声明或定义定义了一个类型为$T$的命名$x$，包涵一个值或方法声明定义了类型$t'$的方法$x$.
- 如果$T <: T'$.一个方法声明或定义定义了一个类型为$T$的命名$x$，包涵一个方法声明定义了类型$t'$的方法$x$.
- 如果$T \equiv T'$，则类型别名`type $t$[$T_1$ , … , $T_n$] = $T$`包含类型别名`type $t$[$T_1$ , … , $T_n$] = $T'$`。
- 如果$L' <: L$ 并且 $U <: U'$，类型声明`type $t$[$T_1$ , … , $T_n$] >: $L$ <: $U$`包含类型声明`type $t$[$T_1$ , … , $T_n$] >: $L'$ <: $U'$`。
- 如果$L <: t <: U$一个绑定到一个类型名称$t$的类型或类定义包含一个抽象类型声明`type t[$T_1$ , … , $T_n$] >: L <: U`。


#### 最小上限和最大下限

$(<:)$ 关系在类型之间形成一种顺序,即它具有传递性和反射性。这允许我们根据该顺序定义一组类型的*最小上限*和*最大下限*。一组类型的最小上限或最大下限并不是一定存在的。例如，以下类定义：
...
类型`A[Any], A[A[Any]], A[A[A[Any]]], ...`构成了`B`和`C`的上界的递减序列。最小上限将是该序列的极限，这个极限不是Scala类型。由于这样的情况通常无法检测，因此如果指定了一个最小上界或最大下界的类型，并且这个边界可能复杂度超过编译器设定的限制[^4]的话就可能被Scala编译器拒绝。
...
在某些情况下，Scala使用更一般的一致性关系。类型$S$*弱一致性* 符合$T$类型，写入$S <:_w T$.如果$S <: T$或者$S$和$T$都是原始的数字类型，并且在下面的顺序中$S$ 在 $T$之前。_
...
如果类型包含类型参数或类型变量，则称为*泛型*。*类型擦除* 是从(通用)类型到非泛型类型的映射。我们写$|T|$来删除$T$类型。擦除映射定义如下：
